...
class MetaDataMDX(InMemoryMetaData):
    ...
    def _is_metadata_fresh(self, item):
        # expiration_date is initialized to empty dict {}, so there is a race condition here
    ...
    def __getitem__(self, item):
        ...
        if item not in cache:
            ...
            # The pop here is contributing to race condition so comment out until after refactor.
            # old_entity = cache.pop(item)
            entity = self._fetch_metadata(item)
        else:
            entity = cache[item]
