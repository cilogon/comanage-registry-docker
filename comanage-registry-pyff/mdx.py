    ...
    def request(self, **kwargs):
        ...
        with self.lock.readlock:
            if ext == 'ds':
                ...
                try:
                    template_name_base = os.path.split(path)[1].split('.')[0]
                    if template_name_base in config.discovery_templates:
                        template_name = "{}.html".format(template_name_base)
                    else:
                        template_name = config.ds_template
                except Exception:
                    template_name = config.ds_template
                return render_template(template_name, **pdict)
            elif ext == 's':
